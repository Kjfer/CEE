...
import { UserPlus, Mail, Shield, Key, X, Lock, Unlock } from 'lucide-react';
import { Button } from '@/components/ui/button';
import { Input } from '@/components/ui/input';
import { Label } from '@/components/ui/label';
import { useToast } from '@/hooks/useToast';
import { adminsService, type AdminProfile, type AdminAction } from '@/services/adminsService';
...
function AdminAvatar({ admin }: { admin: AdminProfile }) {
  ...
  return (
    <div className="flex h-8 w-8 shrink-0 items-center justify-center overflow-hidden rounded-full bg-[#682222] text-xs font-bold text-white">
      {admin.avatarUrl
...
export default function AdminsListPage() {
  ...
  const handleConfirmAction = async () => {
    ...
    // La salvaguarda de "nunca sin Super Admin activo" vive en un trigger de
    // BD; si se viola, este mensaje es el que el trigger devuelve tal cual.
    if (error) {
      toastError('No se pudo completar la acción', error);
    } else {
  ...
  const askToggleStatus = (admin: AdminProfile) =>
    setActionData({ isOpen: true, type: 'toggle_status', targetId: admin.id, targetName: admin.name, isActive: admin.is_active });

  const askRoleChange = (admin: AdminProfile, nextIsSuperadmin: boolean) => {
    if (nextIsSuperadmin === admin.is_superadmin) return;
    setActionData({
      isOpen: true,
      type: nextIsSuperadmin ? 'promote' : 'demote',
      targetId: admin.id,
                ...
                {admins.map(admin => (
                  <tr key={admin.id} className="hover:bg-muted/30">
                    ...
                    <td className="px-6 py-4">
                      {user?.is_superadmin ? (
                        <select
                          value={admin.is_superadmin ? 'super_admin' : 'admin'}
                          onChange={(e) => askRoleChange(admin, e.target.value === 'super_admin')}
                          className="rounded-md border border-input bg-background px-2 py-1.5 text-xs font-medium transition-colors duration-[400ms] focus:border-[#682222] focus:outline-none focus:ring-1 focus:ring-[#682222]/40"
                        >
          ...
          <AlertDialogHeader>
            <AlertDialogTitle>
              {actionData.type === 'promote'
                ...
                : actionData.type === 'demote'
                  ? '¿Quitar Super Administrador?'
                  : `¿${actionData.isActive ? 'Inhabilitar' : 'Habilitar'} a ${actionData.targetName}?`}
            </AlertDialogTitle>
            <AlertDialogDescription>
              {actionData.type === 'promote'
                ? `Estás a punto de ascender a ${actionData.targetName} a Super Administrador. Esta acción le dará control total sobre la plataforma.`
                : actionData.type === 'demote'
                  ? `Estás a punto de quitarle el rol de Super Administrador a ${actionData.targetName}. Si es el único Super Admin activo, el sistema rechazará esta acción.`
                  : (() => {
                      const target = admins.find((a) => a.id === actionData.targetId);
                      const base = `Si inhabilitas a ${actionData.targetName}, no podrá volver a iniciar sesión en el panel administrativo hasta que lo vuelvas a habilitar.`;
                      return target?.is_superadmin
                        ? `${base} Si es el único Super Admin activo, el sistema rechazará esta acción.`
                        : base;
                    })()}
            </AlertDialogDescription>
          </AlertDialogHeader>
