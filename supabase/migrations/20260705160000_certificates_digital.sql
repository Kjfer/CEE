-- Certificados digitales v1 (adaptado al esquema remoto de certificates).
...
CREATE TABLE IF NOT EXISTS public.student_enrollments (
  ...
  status text NOT NULL DEFAULT 'in_progress'
    CHECK (status IN ('in_progress', 'completed', 'approved', 'failed')),
  completed_at date,
  approved_at date,
  notes text,
...
-- ── certificates: columnas aditivas ─────────────────────────────────────────
ALTER TABLE public.certificates
  ADD COLUMN IF NOT EXISTS course_name text,
  ADD COLUMN IF NOT EXISTS profile_id uuid REFERENCES public.profiles(id) ON DELETE SET NULL,
  ADD COLUMN IF NOT EXISTS program_id uuid REFERENCES public.programs(id) ON DELETE SET NULL,
  ADD COLUMN IF NOT EXISTS certificate_type text NOT NULL DEFAULT 'course'
...
SET course_name = co.title
FROM public.courses co
WHERE c.course_id = co.id AND c.course_name IS NULL;
...
ALTER TABLE public.certificates ADD CONSTRAINT certificates_status_check
  CHECK (status IN ('draft', 'pending_signature', 'issued', 'signed', 'revoked'));
...
CREATE UNIQUE INDEX IF NOT EXISTS certificates_unique_active_course
  ON public.certificates (student_id, course_id)
  WHERE status IN ('issued', 'signed', 'pending_signature') AND course_id IS NOT NULL AND student_id IS NOT NULL;
...
CREATE POLICY "certificates_storage_read_public" ON storage.objects
  FOR SELECT USING (bucket_id = 'certificates');

DROP POLICY IF EXISTS "certificates_storage_admin_insert" ON storage.objects;
CREATE POLICY "certificates_storage_admin_insert" ON storage.objects
...
CREATE POLICY "certificates_storage_admin_update" ON storage.objects
...
CREATE POLICY "certificates_storage_admin_delete" ON storage.objects
  FOR DELETE USING (bucket_id = 'certificates' AND public.is_admin());

-- ── RPC pública de verificación ─────────────────────────────────────────────
CREATE OR REPLACE FUNCTION public.get_public_certificate(p_code text)
RETURNS json
LANGUAGE sql
STABLE
SECURITY DEFINER
...
AS $$
  ...
  WHERE cert.verification_code = p_code
    AND cert.status IN ('issued', 'signed')
  LIMIT 1;
$$;
